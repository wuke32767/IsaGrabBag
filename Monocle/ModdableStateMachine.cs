...
{
    public class ModdableStateMachine : Component
    {
        private int state;
        ...
        public ModdableStateMachine(int initialCount = 10)
            : base(true, false)
        {
            PreviousState = state = -1;

            begins = new List<Action>(initialCount);
            begins.AddRange(new Action[initialCount]);
            updates = new List<Func<int>>(initialCount);
            updates.AddRange(new Func<int>[initialCount]);
            stateNames = new List<string>();
            stateNames.AddRange(new string[initialCount]);
            ends = new List<Action>(initialCount);
            ends.AddRange(new Action[initialCount]);
            coroutines = new List<Func<IEnumerator>>(initialCount);
            coroutines.AddRange(new Func<IEnumerator>[initialCount]);
        ...
        public bool HasState(int state) { return stateNames.Count < state; }
        public bool HasState(string name) { return stateNames.Contains(name); }
        ...
        public int CreateNewState(Func<int> onUpdate, string name = "", Func<IEnumerator> coroutine = null, Action begin = null, Action end = null)
        {
        ...
        public static implicit operator int(ModdableStateMachine s)
        {
            return s.state;
        }
